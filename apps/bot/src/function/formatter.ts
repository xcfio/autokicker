...
export function Time(date: number | Date = new Date(), tz = "BST", format = "en-UK") {
    ...
    return new Intl.DateTimeFormat(format, options).format(date).replace(",", "").toUpperCase()
}
...
export function ObjectString(arg: Record<string, any>, convert?: boolean, separator = "\n") {
    const StringArray: Array<string> = []

    for (const key in arg) {
        const ReadableKey = key.startsWith("xp")
            ? key.replaceAll("_", " ")
            : key.slice(0, 1).toUpperCase() + key.slice(1).replaceAll("_", " ")

        const value =
            convert && typeof arg[key] === "number"
